<template>
    <router-view>
          ...
          <div class="form">
            <form v-on:submit.prevent>
              ...
              <div class="d-flex justify-content-between">
                ...
                />
                <button @click="addPost()" class="public btn btn-primary">
                  Publish
                </button>
              </div>
            </form>
          ...
          >
              ...
              >
                <div class="d-flex flex-row align-items-center px-2">
                  ...
                  <div class="d-flex flex-column flex-wrap ml-2">
                    <span class="font-weight-bold last_nameUser"
                      >{{ post.first_name }} {{ post.last_name }}</span
                    ><span class="text-black-50"
                      >Posted the {{ formatDate(post.date) }}</span
                    >
                  </div>
                  <img
                    class="deletePost"
            ...
            <div class="block-com display">
              ...
              >
                ...
                <div v-if="post.postId === comment.postId" class="commentary">
      ...
      <div v-else class="noPost">
        <p class="text-noPost">No post here yet ...</p>
      </div>
    </router-view>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      showComment(event) {
        ...
        if (path.matches(".display")) {
          path.classList.remove("display");
        } else {
          path.classList.add("display");
        }
      },
      upload2(event) {
        ...
        axios
          ...
          .then(function (response) {
            ...
            for (const likedPost of likedPosts) {
              self.likedPost.push(likedPost.postId);
            }
          })
          .catch(function (error) {
            console.log(error);
        ...
        if (this.newComment) {
          ...
          axios
            ...
            .then(function (response) {
              ...
              pathClass.classList.remove("display");
      ...
      getPost() {
        ...
        axios
          ...
          .then((response) => {
            this.postsReceived = response.data;
            this.first = this.postsReceived[0];
            if (this.posts != this.postsReceived) {
              this.posts = this.postsReceived;
            }
          })
          .catch(function (error) {
